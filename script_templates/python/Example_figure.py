...
#
## Check if TP is used
#if 'P' in locals() and 'data' in locals():
#    print('TP is used.')
#
#    plt.figure(2)
#
#    plt.subplot(2, 2, 1)
#    # Pore pressure image
#    plt.imshow(data.P / 1e6, extent=[0, (data.nt - 1) * data.dt, data.x.min() / 1e3, data.x.max() / 1e3])
#    plt.xlabel('Time (s)')
#    plt.ylabel('Along strike distance (km)')
...
#    plt.subplot(2, 2, 2)
#    # Temperature image
#    plt.imshow(data.T, extent=[0, (data.nt - 1) * data.dt, data.x.min() / 1e3, data.x.max() / 1e3])
#    plt.xlabel('Time (s)')
#    plt.ylabel('Along strike distance (km)')
#    plt.colorbar().set_label('Temperature (K)')
#
#    plt.subplot(2, 2, 3)
#    plt.plot(np.arange(data.nt) * data.dt, data.P[p1, :], label=f'x={p1}')
#    plt.plot(np.arange(data.nt) * data.dt, data.P[p2, :], label=f'x={p2}')
#    plt.plot(np.arange(data.nt) * data.dt, data.P[p3, :], label=f'x={p3}')
#    plt.xlabel('Time (s)')
...
#
#plt.subplot(2, 2, 4)
#plt.plot(np.arange(data.nt) * data.dt, (data.st[p1, :] + data.st0[p1, :]) / 1e6, label=f'x={p1}')
#plt.plot(np.arange(data.nt) * data.dt, (data.st[p2, :] + data.st0[p2, :]) / 1e6, label=f'x={p2}')
#plt.plot(np.arange(data.nt) * data.dt, (data.st[p3, :] + data.st0[p3, :]) / 1e6, label=f'x={p3}')
#plt.xlabel('Time (s)')
#plt.ylabel('Shear stress (MPa)')
#plt.legend()
#
...
#
#    plt.subplot(2, 2, 4)
#    plt.plot(np.arange(data.nt) * data.dt, data.T[p1, :], label=f'x={p1}')
#    plt.plot(np.arange(data.nt) * data.dt, data.T[p2, :], label=f'x={p2}')
#    plt.plot(np.arange(data.nt) * data.dt, data.T[p3, :], label=f'x={p3}')
#    plt.xlabel('Time (s)')
#    plt.ylabel('Temperature (K)')
#    plt.legend()
#
#    plt.savefig('./figs/pressure_temp.png')
#
#plt.show()
